...
##构建成功后的zip包
distribution/target/distribution-bin/dao-binary-bin.zip
##构建成功后包所在路径的前缀
distribution/target/distribution-bin/

#此目录要在soft目录下
dao-binary-bin_jenkins_archive
...
# (按需修改) 应用安装根目录，绝对路径
export project_dir=/home/framework/soft/dao-binary-bin
# (按需修改) 应用安装包名称，压缩包形式
export app_file_name=dao-binary-bin.zip
# (按需修改) 应用启动命令，需是完整的可执行的命令
export app_start_cmd='/home/framework/soft/dao-binary-bin/bin/dao-binary start'
# (按需修改) 应用停止命令，需是完整的可执行的命令
export app_stop_cmd='/home/framework/soft/dao-binary-bin/bin/dao-binary stop'
